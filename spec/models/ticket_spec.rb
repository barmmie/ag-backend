...
RSpec.describe Ticket, type: :model do
  describe 'Calculating lowest possible price for tickets' do
    before do
      ...
      FactoryGirl.create(:ticket, price: 17, quantity: 3, splits: [1, 2, 3], event: @event)
    ...
    it 'calculates lowest possible price for 3 tickets' do
      expect(@event.tickets.lowest_total_price_for(3).to_i).to eql(47)
    end

    it 'calculates lowest possible price for 1 ticket' do
      expect(@event.tickets.lowest_total_price_for(1).to_i).to eql(17)
    end

    it 'calculates lowest possible price for 2 tickets' do
      expect(@event.tickets.lowest_total_price_for(2).to_i).to eql(30)
    end

    it 'calculates lowest possible price for 5 tickets' do
      expect(@event.tickets.lowest_total_price_for(5).to_i).to eql(81)
    end
  end
end
