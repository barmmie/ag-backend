...
RSpec.describe Ticket, type: :model do
  describe 'Calculating lowest possible price for tickets' do
    ...
    it 'calculates lowest possible price for 3 tickets' do
      expect(@event.tickets.lowest_total_price_for(3).to_i).to eql((ticket2.price * 2) + (ticket1.price))
    end

    it 'calculates lowest possible price for 1 ticket' do
      expect(@event.tickets.lowest_total_price_for(1).to_i).to eql(ticket1.price)
    end

    it 'calculates lowest possible price for 2 tickets' do
      expect(@event.tickets.lowest_total_price_for(2).to_i).to eql(ticket2.price * 2)
    end

    it 'calculates lowest possible price for 5 tickets' do
      expect(@event.tickets.lowest_total_price_for(5).to_i).to eql(ticket1.price * 3 + ticket2.price * 2)
    end
  end
end
